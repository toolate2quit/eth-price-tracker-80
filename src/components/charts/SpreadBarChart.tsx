...
const SpreadBarChart: React.FC<SpreadBarChartProps> = ({ 
  ...
  const handleMouseMove = (e: any) => {
    if (e && e.activePayload && e.activePayload.length) {
      ...
      console.log('Hover - time:', time, 'value:', value);
      setActiveData({ time, value });
    } else {
      console.log('Hover - no payload');
    }
  };
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        {/* Visible crosshair lines when hovering */}
        {activeData && (
          <>
            <ReferenceLine x={activeData.time} stroke="#ff0000" strokeDasharray="3 3" strokeWidth={2} />
            <ReferenceLine y={activeData.value} stroke="#ff0000" strokeDasharray="3 3" strokeWidth={2} />
          </>
        )}
        
        {/* Include Tooltip with invisible content to help trigger mouse events */}
        <Tooltip 
          cursor={false}
          content={() => null}
